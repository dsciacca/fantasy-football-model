...
## Data Cleaning
Data from [PFF](https://www.profootballfocus.com/) came very clean from the get-go and didn't require any further cleaning for my initial needs. `nflscrapR` data, however, did take some considerable cleaning. My entire process, which is thoroughly explained, can be seen in the [Fantasy Football Regression Model - nflscrapR data cleaning](Fantasy%20Football%20Regression%20Model%20-%20nflscrapR%20data%20cleaning.ipynb) Jupyter notebook, and the highlights are below:
...
After cleaning my data I moved on to Exploratory Data Analysis, or EDA. Here, I explored both the `nflscrapR` data as well as the `PFF` data. __NOTE:__ Since `PFF` data requires a subscription to access, I have redacted any output of this raw data from the [Fantasy Football Regression Model - EDA](Fantasy%20Football%20Regression%20Model%20-%20EDA.ipynb) notebook, but I have included some of the code in my explanations here. 
